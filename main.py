...
known_project_ids = map( lambda x: x["gid"], known_projects)
...
def get_events_from_resources(resource_ids):
    sync_tokens = {resource_id: get_events_sync_token(resource_id) for resource_id in resource_ids}
...
def process_events():
    ...
    while True:
        next_event = next(event_generator)
